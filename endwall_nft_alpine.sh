...
#
# Changes:     - Translate endwall_nft.sh from Debian to Alpine Linux
#              - Translate endwall.sh from iptables to nftables
...
# $ chmod u+rwx endwall_nft_alpine.sh          # changer permisions to allow script execution
# $ su                              # become root
# # ./endwall_nft_alpine.sh                    # execute/run the file
#
#  If the firewall fails (bad interface pickup or bad ipv4 pickup) then run ./endwall_nft_alpine.sh --open to return to open policies
# $ ./endwall_nft_alpine.sh --open
...
int_if1=$(ip link | grep -a "state " | awk -F: '{ if (FNR==2) print $2}' | xargs echo -n)
int_if2=$(ip link | grep -a "state " | awk -F: '{ if (FNR==3) print $2}' | xargs echo -n)

# Grab Gateway Information
gateway_ip=$(ip route | awk '/via/ {print $3}' | head -1)
...
if [ "$state" = "closed" ]; then
    ...
    for int_if in "$int_if1" "$int_if2"; do

        if [ "$int_if" = "$int_if1" ]; then
            int_mac="$int_mac1"
            int_ip="$int_ip1"
            int_ipv6="$int_ip1v6"

        elif [ "$int_if" = "$int_if2" ]; then
            int_mac="$int_mac2"
            int_ip="$int_ip2"
            int_ipv6="$int_ip2v6"
        fi
